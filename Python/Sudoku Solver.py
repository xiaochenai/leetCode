class Solution:
	...
	def solveSudoku(self, board):
		self.solveSudokus(board,0,0)
	def solveSudokus(self,board,row,col):
		if row == 9:
			return True
		if col == 8:
			nextRow = row +1
			...
			nextCol = col+1
		if board[row][col] != ".":
			return self.solveSudokus(board,nextRow,nextCol)
		for k in range(1,10):
			if self.canPut(board,str(k),row,col):
				board[row][col] = str(k)
				if self.solveSudokus(board,nextRow,nextCol):
					return True
				board[row][col]="."
		return False
